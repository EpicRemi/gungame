...
public partial class ExamplePlayer : PlayerBase
{
    ...
    public override void OnKilled()
    {
        ...
        var attacker = LastAttacker as ExamplePlayer;
        ...
        if (attacker != null && LastDamage.Weapon is WeaponBase weapon && GameManager.Current is ExampleGame game)
        {
            ...
            if (Game.IsServer && attacker != this) 
            {
                attacker.score++;
                attacker.Inventory.DeleteContents();
                attacker.Inventory.Add(new SWB_WEAPONS.Bayonet());
                attacker.SwitchToBestWeapon();
                switch (attacker.score)
                {
                    case 0:
                        attacker.Inventory.Add(new SWB_EXPLOSIVES.RPG7());
                        attacker.SwitchToBestWeapon();
                        break;
                    case 1:
                        attacker.Inventory.Add(new SWB_WEAPONS.L96A1());
                        attacker.SwitchToBestWeapon();
                        break;
                    case 2:
                        attacker.Inventory.Add(new SWB_WEAPONS.FAL());
                        attacker.SwitchToBestWeapon();
                        break;
                    case 3:
                        attacker.Inventory.Add(new SWB_WEAPONS.UMP45());
                        attacker.SwitchToBestWeapon();
                        break;
                    case 4:
                        attacker.Inventory.Add(new SWB_WEAPONS.SPAS12());
                        attacker.SwitchToBestWeapon();
                        break;
                    case 5:
                        attacker.Inventory.Add(new SWB_WEAPONS.DEAGLE());
                        attacker.SwitchToBestWeapon();
                        // ActiveChild = Children.;
                        break;
                }
                Log.Info($"{attacker.score}");
            }
        }
